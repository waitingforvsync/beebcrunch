        ...
        for (uint32_t k = i; k < j; k++) {
            ...
            if (is_lit) {
                ...
                bits += table_cost(&tables->len, t.length)
                      + table_cost(coding_off_table(tables, t.length), t.offset);
            }
        }
        i = j;
    }
                             ...
                             const coding_tables *tables, rc_arena *arena)
{
    ...
    write_coding_tables(&w, tables, 0);
    ...
    while (i < tokens.num) {
        ...
        for (uint32_t k = i; k < j; k++) {
            ...
            if (is_lit) {
                ...
                write_value(&w, coding_off_table(tables, t.length), t.offset);
            }
            total += t.length;
        }
        i = j;
...
v5_compress_result v5_compress(rc_view_bytes in, rc_arena *arena, rc_arena scratch)
{
    ...
    // result still stands.
    coding_tables tables = seed_coding_tables(0, 1);
    for (uint32_t iter = 0; iter < v5_iterations; iter++) {
        ...
        uint32_t bits = coding_transmit_bits(&next, 0) + data_bits(parsed.tokens.view, &next);
        if (bits < best_bits) {
            best_bits = bits;
...
v5_decompress_result v5_decompress(rc_view_bytes comp, rc_arena *arena)
{
    ...
    coding_tables_result tables = read_coding_tables(&r, 0, 1);
    ...
    while (out.num < len) {
        ...
        for (uint32_t k = 0; k < count; k++) {
            ...
            value_result offset = read_value(&r, coding_off_table(&tables.tables, length.value));
                         ...
                         uint32_t pos, uint32_t *best, rc_arena *arena)
{
    ...
    if (d != 0
        ...
        && table_cost(&tables->off[0], d) < table_big_cost) {
        rc_array_token_push(seq, (token) {
            .length = 1,
            .offset = (uint16_t)d,
        }, arena);
