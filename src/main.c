...
static int cmd_bench(const char *corpus_dir)
{
    ...
    for (uint32_t k = 0; k < sizeof corpus_files / sizeof corpus_files[0]; k++) {
        ...
        v6_compress_result c6 = v6_compress(f.contents.view, &arena, scratch);
        v6_decompress_result d6 = v6_decompress(c6.data.view, &arena);
        if (!d6.ok || d6.data.num != f.contents.num
            || memcmp(d6.data.data, f.contents.data, f.contents.num) != 0) {
            fprintf(stderr, "bench: %s failed v6 round trip\n", path);
            rc = 1;
            continue;
        }
